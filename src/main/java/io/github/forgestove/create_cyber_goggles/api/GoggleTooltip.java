...
public final class GoggleTooltip {
	...
	public static final ResourceLocation NATIVE = ResourceLocation.fromNamespaceAndPath("create", "native");
	@FunctionalInterface
	...
	public static void registerBefore(Class<? extends BlockEntity> target, ResourceLocation anchorId, ResourceLocation id, Provider provider) {
		REGISTRY.registerBefore(target, anchorId.toString(), check(id), wrap(provider));
	}
	/** 把内容条目插入到指定锚点条目之后（锚点不存在则退化为末尾追加并 WARN）。 */
	public static void registerAfter(Class<? extends BlockEntity> target, ResourceLocation anchorId, ResourceLocation id, Provider provider) {
