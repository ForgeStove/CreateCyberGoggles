...
public abstract class ConfigEntry extends Entry<ConfigEntry> {
	...
	public Tooltip getHoveredWidgetTooltip(int mouseX, int mouseY) {
		for (var child : children())
			if (child instanceof AbstractWidget widget && widget.visible && isMouseOverWidget(widget, mouseX, mouseY)) try {
				var field = AbstractWidget.class.getDeclaredField("tooltip");
				field.setAccessible(true);
				var holder = (WidgetTooltipHolder) field.get(widget);
				var tooltip = holder.get();
