...
public final class KeybindValueConfigEntry<C> extends ValueConfigEntry<C, Key> {
	...
	private Component getConflictTooltip() {
		...
		for (var i = 0; i < conflictUsages.size(); i++) {
			if (i > 0) usedBy.append(Component.literal(", "));
			usedBy.append(conflictUsages.get(i));
		}
	...
	public void renderContent(@NotNull GuiGraphics gui, int mouseX, int mouseY, boolean hovered, float delta) {
		renderGui(gui, getY(), getX(), getWidth(), mouseX, mouseY, delta, undoButton, resetButton, bindButton);
	}
