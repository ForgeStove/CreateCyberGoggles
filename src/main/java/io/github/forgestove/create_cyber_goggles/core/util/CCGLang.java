...
public class CCGLang {
	...
	public static @NotNull CCGLangBuilder fluid(@NotNull FluidStack fluid, long capacityMb) {
		var marker = Component.empty();
		TooltipComponentUtil.FLUID_ENTRY_MAP.put(marker, new FluidEntryTooltipComponent(fluid, 0, capacityMb));
		return builder().add(marker);
	}
	@SuppressWarnings("unused")
	public static @NotNull CCGLangBuilder fluidList(@NotNull List<FluidStack> fluids, int maxColumns) {
		var marker = Component.empty();
		TooltipComponentUtil.FLUID_LIST_MAP.put(marker, new FluidListTooltipComponent(fluids, 0, maxColumns));
		return builder().add(marker);
	}
}
