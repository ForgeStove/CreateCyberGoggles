...
public class GoggleTooltipUtil {
	...
	public static boolean depot(List<Component> tooltip, DepotItemHandler itemHandler) {
		...
		for (var view : itemHandler) {
			if (view.isResourceBlank() || view.getAmount() <= 0) continue;
			var count = (int) Math.min(Integer.MAX_VALUE, view.getAmount());
			var stack = view.getResource().toStack(1);
	...
	public static boolean basin(
		...
		List<Long> inputCapacities,
		List<Long> outputCapacities
	) {
		...
		if (!inputFluids.isEmpty()) {
			...
			for (var i = 0; i < inputFluids.size(); i++) {
				...
				var capacityMb = i < inputCapacities.size() ? inputCapacities.get(i) : Math.max(1000, ccg$getFluidAmount(fluidStack));
		...
		if (!outputFluids.isEmpty()) {
			...
			for (var i = 0; i < outputFluids.size(); i++) {
				...
				var capacityMb = i < outputCapacities.size() ? outputCapacities.get(i) : Math.max(1000, ccg$getFluidAmount(fluidStack));
				CCGLang.fluid(fluidStack, capacityMb).forGoggles(tooltip, 1);
			}
		}
		return true;
	}
	private static int ccg$getFluidAmount(Object fluidStack) {
		if (fluidStack == null) return 0;
		try {
			return (int) fluidStack.getClass().getMethod("getAmount").invoke(fluidStack);
