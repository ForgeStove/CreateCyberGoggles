...
public class TooltipUtil {
	public static void kinetic(List<Component> tooltip, @NotNull KineticBlockEntity kbe, float stress, float capacity) {
		...
		if (StressImpact.isEnabled()) {
			...
			if (!Mth.equal(stressAtBase, 0)) {
				...
				Lang.number(stressAtBase * Math.abs(speed))
					.translate("generic.unit.stress")
					.style(AQUA)
					.space()
					.add(Lang.translate("gui.goggles.at_current_speed").style(DARK_GRAY))
					.forGoggles(tooltip, 1);
			}
		}
		Lang.translate("gui.speedometer.title").style(GRAY).forGoggles(tooltip);
		...
		if (speed == 0)
			...
			if (remainingCapacity != capacity) stressTip.text(GRAY, " / ").add(Lang.number(capacity).add(su).style(DARK_GRAY));
	...
	public static void generatingKinetic(List<Component> tooltip, @NotNull GeneratingKineticBlockEntity gkbe) {
		...
		if (!Mth.equal(stressBase, 0)) {
			...
			Lang.translate("tooltip.capacityProvided").style(GRAY).forGoggles(tooltip);
			...
			Lang.number(Math.abs(stressBase * speed))
				.translate("generic.unit.stress")
				.style(AQUA)
				.space()
				.add(Lang.translate("gui.goggles.at_current_speed").style(DARK_GRAY))
	...
	public static void cannon(List<Component> tooltip, @NotNull SchematicannonBlockEntity sbe) {
		...
		Lang.translate("schematicannon.status." + sbe.statusMsg).style(GOLD).forGoggles(tooltip);
		...
		if (sbe.hasCreativeCrate) {
			Lang.translate("gui.schematicannon.gunpowderLevel", "" + 100).forGoggles(tooltip);
			...
			Lang.translate("gui.schematicannon.gunpowderLevel", fillPercent).forGoggles(tooltip);
			Lang.translate("gui.schematicannon.shotsRemaining", CCGLang.number(BLUE, shotsLeft)).style(GRAY).forGoggles(tooltip);
			if (shotsLeftWithItems != shotsLeft)
				Lang.translate("gui.schematicannon.shotsRemainingWithBackup", CCGLang.number(BLUE, shotsLeftWithItems))
					.style(GRAY)
					.forGoggles(tooltip);
		}
