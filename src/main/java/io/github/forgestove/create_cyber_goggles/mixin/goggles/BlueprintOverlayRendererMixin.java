...
public abstract class BlueprintOverlayRendererMixin {
	...
	private static void renderOverlay(Minecraft mc, GuiGraphics gui, CallbackInfo ci, @Local(name = "x") int x, @Local(name = "y") int y) {
	...
	@WrapOperation(
		method = "renderOverlay", at = @At(
		value = "INVOKE",
		target = "Lnet/minecraft/client/gui/screens/inventory/tooltip/TooltipRenderUtil;renderTooltipBackground"
			+ "(Lnet/minecraft/client/gui/GuiGraphics;IIIILnet/minecraft/resources/Identifier;)V"
	)
	)
	private static void renderOverlay(GuiGraphics gui, int x, int y, int width, int height, Identifier sprite, Operation<Void> original) {
